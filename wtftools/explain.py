...
_RULES: List[Tuple[Callable[[CheckResult], bool], SuggestionAdvice]] = [
    ...
    (
        ...
        "Run queue depth exceeds CPU count. Combine with `wtf audit --check iowait psi` "
        "to separate CPU-bound from IO-bound. `wtf info` (TOP BY CPU) shows the consumer."
    ),
    (
    ...
    (
        ...
        lambda r: (
            "Failed unit(s): " + ", ".join(r.detail) + ". "
            "Inspect each with `wtf services <name>` or `systemctl status <name>` "
    ...
    (
        ...
        lambda r: (
            ...
            "Cause is in: " + ", ".join(r.detail[:3]) + ". "
            "Run `systemctl status <name>` for the last fail reason."
        ),
    ),
    (
    ...
    (
        ...
        lambda r: (
            ...
            "Encrypt) or your CA flow. Most urgent: " +
            (r.detail[0] if r.detail else "see audit detail") + ". "
            "Automate renewal + reload hook for the consumer (nginx/haproxy/etc)."
        ),
    ),
    (
        lambda r: r.name == "TCP retransmits" and r.status in ("warn", "fail"),
        "Network is dropping packets. Check switch port errors (`ethtool -S eth0`), "
        "MTU mismatch, congestion. `ss -ti` shows per-connection retrans counts."
    ),
    (
        lambda r: r.name == "network errors" and r.status in ("warn", "fail"),
        "NIC accumulated rx/tx errors or drops. Likely a hardware/cable issue or "
        "buffer overflow under burst. `ethtool -S <iface>`, `ip -s link show <iface>`."
    ),
    (
    ...
    (
        ...
        "journald has grown large. `journalctl --vacuum-size=2G` or "
        "`--vacuum-time=7d`. To bound permanently: edit "
        "/etc/systemd/journald.conf → `SystemMaxUse=4G`."
    ),
    (
        lambda r: r.name == "reboot required" and r.status in ("warn", "fail"),
        "Pending kernel/library update needs a reboot. Schedule the reboot to "
        "pick up security fixes. The pkg list shows what triggered it."
    ),
    (
        lambda r: r.name == "system state" and r.status in ("warn", "fail"),
        "systemd reports 'degraded' — at least one unit is failed. Use "
        "`wtf audit --check failed-units` for the list, then `wtf services <name>`."
    ),
    (
    ...
    (
        ...
        "Recent kernel error lines — could indicate hardware (memory, disk), "
        "driver, or filesystem issue. `journalctl -k -p err --since '24h ago'`."
    ),
    (
        lambda r: r.name == "open file descriptors" and r.status in ("warn", "fail"),
        "File descriptor pressure. Find offenders: "
        "`for p in /proc/[0-9]*; do echo \"$(ls $p/fd 2>/dev/null | wc -l) $p\"; done | sort -n | tail`."
    ),
    (
        lambda r: r.name == "process count" and r.status in ("warn", "fail"),
        "PID table filling — fork-bomb, runaway service, or insufficient pid_max. "
        "Find the parent: `ps -eo pid,ppid,comm --sort=-pid | head -20`."
    ),
    (
        lambda r: r.name.startswith("plugin:") and r.status in ("warn", "fail"),
        "Custom plugin reported a problem. Re-run the plugin manually to inspect "
        "its full output."
    ),
]

_FALLBACK = (
    "No built-in advice for this check yet. Use `wtf audit -v --check <name>` "
    "to see the full detail, or pipe `wtf explain --prompt` to an LLM."
)
...
def suggest(result: CheckResult) -> Suggestion:
    ...
    for predicate, advice in _RULES:
        if predicate(result):
            ...
            return Suggestion(name=result.name, status=result.status,
                              message=result.message, advice=text)
    return Suggestion(name=result.name, status=result.status,
