...
- **Аудит здоровья** — 35+ проверок (диск, память, swap, нагрузка, PSI, OOM-убийства,
...
- **Для скриптов** — `-f json` есть у каждой команды, а `-f plain` (с разделением табуляцией) —
  у представлений ресурсов и аудита; JSON несёт поле `schema_version`, чтобы скрипты переживали обновления — для grep / awk / jq.
